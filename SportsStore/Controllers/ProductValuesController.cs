...
namespace SportsStore.Controllers
{
  ...
  public class ProductValuesController : Controller
  {
    ...
    public IActionResult UpdateProduct(long id, [FromBody] JsonPatchDocument<ProductData> patch)
    {
      ...
      //apply the operations in the JSON Patch Document to the ProductData object
      //which will pass on those changes to the Product object
      //the ApplyTo method updates the controller's model state to record any errors in the JSON Patch document
      //such as an operation on a property that isn't defined by the ProductData class
      patch.ApplyTo(pdata, ModelState);

      //before storing the changes in the database, check if the ModelState.IsValid property
      //and explicitly validate the ProductData object to make sure that the changes have not created
      //an invalid outcome
      if(ModelState.IsValid && TryValidateModel(pdata)){
        ...
        //if there are no validation errors, the changed are written to the database by calling the context object's
        //SaveChanges and the modifies Product object is returned to the client
        context.SaveChanges();
        return Ok();
      }
      else{
